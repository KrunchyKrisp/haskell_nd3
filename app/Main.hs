...
overlaps (Rectangle x1 y1 a1 b1) (Rectangle x2 y2 a2 b2) = x1 + a1 >= x2 && x1 <= x2 + a2 && y1 + b1 >= y2 && y1 <= y2 + b2
...
overlaps (Circle x1 y1 r1) (Rectangle x2 y2 a2 b2) = (x1 - closestX)^2 + (y1 - closestY)^2 < r1^2
    where --closest (x,y) of the rectangle to center of the circle
